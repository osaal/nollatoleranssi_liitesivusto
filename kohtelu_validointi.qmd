...
```{r}
#| label: kohtelu_validointi-XX-kuvailevattallennus
...
malli.1 <- cfa(
  ...
  estimator = "MLR", # Robusti ML aiheuttaa ongelmia, kokeillaan siirtyä robustiin DLWS-estimaattoriin
  missing = "fiml" # Robusti DLWS hyväksyy listwise, pairwise tai imputoidut datasetit
)
...
```{r}
#| label: kohtelu_validointi-XX-residuaalit-tallennus
#| code-summary: "Standardisoitujen residuaalikovarianssin tallentaminen"

write.csv2(
  x = lavResiduals(malli.1)$cov.z,
  file = "output/validointiartikkeli_malli2b_stdresid.csv"
)
...
Tarkistimme seuraavaksi virhekorrelaatiot. Kaksi suurinta pudotusta khii neliö-testin suureessa tulisi muuttujaparien `k3_5 ~~ k12_5` ja `k3_7 ~~ k12_7` vapaasta virhekorrelaatiosta. Näiden odotetut virhekorrelaatiot ovat 0,076 ja 0,108.

### Erotteluvaliditeetti

```{r}
#| label: fig-kohtelu_validointi-validiteettikorrelaatio
...
```{r}
#| label: kohtelu_validointi-XX-ulkoinenvaliditeetti
#| code-summary: "Ulkoisen validiteetin testaus"

cfa.malli.ulkval <- '# kohtelumuuttujat
                    f1 =~ start(0.8)*k3_1 + start(0.5)*k3_2 + start(0.7)*k3_3 + start(0.2)*k3_4 + start(0.2)*k3_5 + start(0.8)*k3_6 + start(0.7)*k3_7
                    f2 =~ start(0.9)*k12_1 + start(0.6)*k12_2 + start(0.9)*k12_3 + start(0.8)*k12_4 + start(0.8)*k12_5 + start(0.8)*k12_6 + start(0.6)*k12_7
...
summary(malli.ulkval, fit.measures = TRUE, standardized = TRUE, ci = TRUE)

```

Malli sopii rajallisesti hyväksyttävästi aineistoon ($\text{RMSEA} = .053, \text{CFI} = .900, \text{SRMR} = .076$).
...
Muuttujan `k12_7` vapauttaminen pienentää khii neliö -suuretta 41 pistettä. Pienennys on merkitsevä (p \< .0001). Kaikki sopivuusindeksit parantuvat uudessa mallissa ($\text{RMSEA} = .072, \text{CFI} = .943, \text{SRMR} = .051, \text{AIC} = 14096, \text{BIC} = 14298$), mutta eivät vielä täysin täytä yleisiä raja-arvoja hyväksyttävälle mallille.
...
Virheresiduaalien vapauttaminen tuottaa selkeästi paremmin istuvan mallin. Khii neliö -suure laskee noin 80 pistettä, ja ero on erittäin merkitsevä (p \< ,0001). Kaikki globaalit sopivuusindeksit parantuvat alkuperäisestä mallista ($\text{RMSEA} = .066, \text{CFI} = .953, \text{SRMR} = .066, \text{AIC} = 14053, \text{BIC} = 14268$), ja kaksi kolmesta sopivuusindeksistä täyttävät nyt raja-arvot.
...
Syrjintämuuttujien (`k3_7`, `k12_7`) poisjättäminen mallista tuottaa selkeästi paremmin istuvan mallin. Kaikki globaalit sopivuusindeksit parantuvat lievästi tai pysyvät samoina ($\text{RMSEA} = .072, \text{CFI} = .949, \text{SRMR} = .052$). Koska malli ei ole alkuperäisen mallin alamuoto (muuttujat eivät täsmää määrässään), AIC- ja BIC-arvoja ei voida verrata keskenään.
...
Bifaktorimallia ei saada sovitettua ML-menetelmällä Heywood-tapauksen takia: muuttujan `k3_1` residuaalivarianssi on negatiivinen. Malli voidaan kuitenkin sovittaa DLWS-menetelmällä käyttäen parittaista puuttuvien vastausten poistomenetelmää.

Mallin globaalit istuvuusindeksit parantuvat perusmalliin verrattuna ($\text{RMSEA} = .055 [90 \% \text{ CI } .047, .064], \text{CFI} = .991, \text{SRMR} = .071$)

### Eksploratiivinen analyysi: Bifaktori ilman syrjintää

```{r}
#| label: kohtelu_validointi-XX-bifaktorieisyrj-mallimaaritelma
#| code-summary: "Toisen bifaktorimallin määrittely"

cfa.malli.2b  <- '# latentit muuttujat
                f1 =~ k3_1 + k3_2 + k3_3 + k3_4 + k3_5 + k3_6
                f2 =~ k12_1 + k12_2 + k12_3 + 0.4*k12_4 + k12_5 + k12_6
                
                # yleinen faktori
                gen =~ k3_1 + k3_2 + k3_3 + k3_4 + k3_5 + k3_6 + k12_1 + k12_2 + k12_3 + k12_4 + k12_5 + k12_6
                '

```

```{r}
#| label: kohtelu_validointi-XX-bifaktorieisyrjmalli
#| code-summary: "Toisen bifaktorimallin estimointi"
...
summary(malli.2b, fit.measures = TRUE, standardized = TRUE, ci = TRUE)

```

Malli synnyttää toisen Heywood-tilanteen, joka ratkaistiin rajaamalla muuttujan `k12_4` lataus EFA-analyysissa todettuun arvoon `0.4`.

Mallin globaalit sopivuusindeksit ovat perusmallia selkeästi parempia ($\text{RMSEA} = .053 [90 \% \text{ CI } .043, .064], \text{CFI} = .994, \text{SRMR} = .066$), ja osittain myös ensimmäistä bifaktorimallia parempia. Erot ensimmäiseen bifaktorimalliin ovat kuitenkin pienet.
...
Koetimme verrata kahden faktorin mallin invarianssia kohderyhmien välillä (kolme kategoriaa). Mallia ei kuitenkaan voitu asettaa vähäisen invarianssin takia.
...
Rakenneinvariantit mallit sopivat suurin piirtein yhtä heikosti kuin yleismalli. Globaalit sopivuusindeksit eivät täytä raja-arvoja (RMSEA = ,068, CFI = ,940, SRMR = ,082).
...
Vahvan invarianssin mallit sopivat samoin kuin heikon invarianssin mallit (p = ,129). CFI-mitan muutos on $\Delta\text{CFI} = -0.007$, mikä hyväksyttävän muutoksen rajan sisällä. AIC nousee erittäin vähän ja BIC laskee hieman.
...
Mallien pohjalta voimme todeta, että vahva invarianssi ei näytä täyttyvän pelastajien ja ensihoitajien välillä. Heikko invarianssi täyttyy ehkä (CFI-muutos on hyväksyttävän rajalla), mutta mallin istuvuus on heikohko (RMSEA = ,076, CFI = ,918, SRMR = ,118). Jos heikko invarianssi hyväksyttäisiin, tulisi myös hyväksyä vahva invarianssi, sillä sen sopivuuden huonontuminen on hyvin pieni verrattuna heikon invarianssin malliin. Tiukka invarianssi ei täyty.

Toisin sanoen: pelastajien ja ensihoitajien välillä faktorilataukset ja lähtökohdat (interseptit) ovat kuta kuinkin invariantit, mutta virhemittauksessa on selkeitä eroja.
...
Tiukan invarianssin malli istuu merkitsevästi huonommin kuin vahvan invarianssin malli (p = ,0007). Muutokset ylittävät myös hyväksyttävien muutosten rajat jokseenkin selkeästi ($\Delta\text{RMSEA} = 0.004, \Delta\text{CFI} = -0.018, \Delta\text{SRMR} = 0.038$). Sekä AIC- että BIC-arvot nousevat selkeästi, osoittaen huonompaa istuvuutta.

Kieliversioiden välillä täyttyy heikon ja vahvan invarianssin vaatimus, eli faktorilataukset sekä interseptit ovat samat suomen- ja ruotsinkielisillä mittareilla. Tiukka invarianssi ei kuitenkaan täyty, eli virhemittaukset eivät ole samat molemmilla kieliversioilla.
...
Rakenneinvariantit mallit istuvat heikohkosti aineistoon (RMSEA = ,094, CFI = ,901, SRMR = ,085).
...
Tiukan invarianssin malli istuu merkitsevästi huonommin kuin vahvan invarianssin malli (p = 0,0000002). Kaikki indeksiarvot muuttuvat enemmän kuin hyväksyttävän muutoksen rajat sallivat ($\Delta\text{RMSEA} = 0.021, \Delta\text{CFI} = -0.073, \Delta\text{SRMR} = 0.066$). AIC- ja BIC-arvot nousevat reilusti.

Malli näyttää vahvasti invariantilta, eli faktorilataukset ja interseptit ovat samoja. Tiukkaa invarianssia malli ei kuitenkaan täytä, eli virhemittaukset eroavat ryhmien välillä.
