...
malli.1 <- cfa(
  ...
  estimator = "WLSMV", # Robusti ML aiheuttaa ongelmia, kokeillaan siirtyä robustiin DLWS-estimaattoriin
  missing = "pairwise", # Robusti DLWS hyväksyy listwise, pairwise tai imputoidut datasetit
  ordered = TRUE
)
...
```
Työuupumuksen latentti muuttuja korreloi organisaatiosisäisen epäasiallisen ja väkivaltaisen kohtelun faktorin kanssa tasolla $r = 0,409$. Organisaatioulkoisen kohtelun faktorin kanssa korrelaatio on pienempi, $r = 0,265$.
...
**\[Malli asettuu hyvin DWLS-menetelmällä, analysoi!\]**
...
**HUOM: Heywood-case, joka korjaantuu rajaamalla muuttujan `k12_4` latauksen arvoon `0.4`, eli lähellä alkuperäistä estimaattia.**
...
**HUOM: Mittausinvarianssia ei voida testata robustilla DWLS-estimaattorilla, sillä data ei riitä siihen.**
...
Kohderyhmien välistä invarianssitestausta ei voitu suorittaa vähäisen varianssin takia. Muut invarianssitestaukset osoittavat kuitenkin, että mittarit ovat vahvasti invariantteja ammattiasemien, kielten ja yhteydenottomenetelmien välillä. Tiukka invarianssi ei täyty missään vertailussa.
